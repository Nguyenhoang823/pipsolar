...
class Pipsolar : public uart::UARTDevice, public PollingComponent {
 public:
  ...
  // Number entities mirror the corresponding QPIRI values for true two-way sync.
  ...
  // 24 V battery voltage dropdowns. These mirror QPIRI values for two-way sync.
  PIPSOLAR_SELECT(battery_bulk_voltage_select, QPIRI)
  PIPSOLAR_SELECT(battery_recharge_voltage_select, QPIRI)
  PIPSOLAR_SELECT(battery_under_voltage_select, QPIRI)
